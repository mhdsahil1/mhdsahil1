...
q = '''query($login:String!,$from:DateTime!,$to:DateTime!){user(login:$login){contributionsCollection(from:$from,to:$to){contributionCalendar{totalContributions weeks{contributionDays{date contributionCount}}}} repositories(first:100,ownerAffiliations:OWNER,privacy:PUBLIC,isFork:false){nodes{name languages(first:10,orderBy:{field:SIZE,direction:DESC}){edges{size node{name}}}}}}}'''
...
for repo in data['repositories']['nodes']:
    for e in repo['languages']['edges']: langs[e['node']['name']] += e['size']
...
lang_lines = ''.join(f'<text x="30" y="{70+i*24}" font-size="15">{esc(n)} {size/lang_total*100:5.1f}%</text>' for i,(n,size) in enumerate(langs))
